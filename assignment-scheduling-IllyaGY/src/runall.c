...
int main() {
   ...
   /* open file to write result for fcfs */
   f_fcfs = fopen("fcfs.all", "w");
   if (f_fcfs == NULL) {
      fprintf(stderr, "Error creating fcfs.all\n");
      return 1;
   }
   fprintf(f_fcfs, "FCFS %s\n", semester);

   /* open file to write result for sjf */
   ...
   /* open file to write result for psjf */
   f_psjf = fopen("psjf.all", "w");
   if (f_psjf == NULL) {
      fprintf(stderr, "Error creating psjf.all\n");
      return 1;
   }
   fprintf(f_psjf, "PSJF %s\n", semester);

   /* open file to write result for rr */
   f_rr = fopen("rr.all", "w");
   if (f_rr == NULL) {
