...
void kproc_init(void) {
    ...
    for(int i = 1; i < 5; i++){
        ...
        char shell_name[16];
        snprintf(shell_name, sizeof(shell_name), "shell%d", i);
        pid = kproc_create(prog_shell, shell_name, PROC_TYPE_USER);        // Attach the process to the TTY using kproc_attach_tty
        if(kproc_attach_tty(pid, i) == -1){
            kernel_log_error("kprock attach error");
        }
