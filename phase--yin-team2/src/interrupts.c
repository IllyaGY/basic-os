...
void pic_irq_enable(int irq) {
    ...
    unsigned char curr_PIC = PIC1_DATA; 
    if(irq > PIC1_END){   
        curr_PIC = PIC2_DATA;
        irq-=PIC1_END; 
    }
    // Read the current mask
    unsigned char current_mask = inportb(curr_PIC); 
    // Clear the associated bit in the mask to enable the IRQ
    current_mask &= ~(1 << irq); 
    // Write the mask out to the PIC
    outportb(curr_PIC, current_mask); 
...
int pic_irq_enabled(int irq) {
    ...
    //FOR DEBUG:
    kernel_log_info("%i", (current_mask & (1 << irq)));
...
void pic_irq_dismiss(int irq) {
    ...
    if(irq > PIC1_END) outportb(PIC2_CMD, PIC_EOI);
